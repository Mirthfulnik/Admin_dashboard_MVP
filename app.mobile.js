...
/** -----------------------------
 *  Mobile UI (burger + report fit-to-screen)
 *  - Enabled when window.__LM_MOBILE__ === true (set by index loader)
 * -----------------------------*/
function initMobileUi_(){
  if (!(window && window.__LM_MOBILE__)) return;
  ...
  function openDrawer_(){ if (drawer) drawer.classList.add("open"); }
  function closeDrawer_(){ if (drawer) drawer.classList.remove("open"); }

  if (burgerBtn) burgerBtn.addEventListener("click", openDrawer_);
  if (drawerClose) drawerClose.addEventListener("click", closeDrawer_);
  if (drawerBackdrop) drawerBackdrop.addEventListener("click", closeDrawer_);

  // Build drawer navigation from existing .tabBtn buttons (desktop nav)
  if (drawerNav){
    drawerNav.innerHTML = "";
    const tabs = $$(".tabBtn");
    for (const t of tabs){
      ...
      b.className = "tabBtn";
      b.dataset.route = t.dataset.route || "";
  ...
  // Wrap report-root into viewport element for scaling
  const reportRoot = document.getElementById("report-root");
  if (reportRoot && !reportRoot.parentElement.classList.contains("reportViewport")){
    const vp = document.createElement("div");
    vp.className = "reportViewport";
    reportRoot.parentNode.insertBefore(vp, reportRoot);
...
// Ensure each .pageSheet is wrapped into a sized container so vertical list
  ...
  try{
    ...
    go_ = function(route){
      _go(route);
      if (route === "report") setTimeout(updateReportScale_, 0);
    };
  }catch(e){
    // ignore if go_ is not writable
  }

  // Initial pass (in case app boots directly on report)
  setTimeout(()=>{ updateReportScale_(); }, 0);
}

// run after base init is in place
initMobileUi_();
