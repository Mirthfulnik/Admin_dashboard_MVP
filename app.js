...
(function(){
  ...
  const state = {
    ...
    brand: "lotus", // "lotus" | "zhara" — persisted per-release in cloud snapshot
    temp: {
      streamsParsed: null,
      streamsPreview: null
    }
  ...
  /** Returns the logo img src for the active brand (used on every PDF report slide). */
  function logoSrc_(){
    if (state.brand === "zhara")    return "assets/logo_zhara.png";
    if (state.brand === "ovokacho") return "assets/ovo.png";
    return "assets/logo_black.png";
  }

  /** Returns the alt text for the active brand logo. */
  function logoAlt_(){
    if (state.brand === "zhara")    return "Zhara Music";
    if (state.brand === "ovokacho") return "Ovokacho";
    return "Lotus Music";
  }

  /** Syncs the brand switcher UI buttons to match state.brand. */
  function syncBrandSwitcher_(){
...
if (!snap) throw new Error("Не удалось загрузить snapshot из облака");
    ...
    r.brand = snap.brand || r.brand || "lotus";
...
function getDemoTotals_(c){
  ...
  reportSelect.addEventListener("change", async ()=>{
    ...
    const rr = getCurrentRelease_();
    if (rr && rr.brand) state.brand = rr.brand;
    renderReport_();
  });
  ...
  function renderEditor_(){
    ...
    // Sync brand switcher to the current release's saved brand
    if (r && r.brand) state.brand = r.brand;
    syncBrandSwitcher_();
  }
...
if (demoRows.length){
  ...
  // Brand switcher: update state.brand + persist on release
  document.querySelectorAll(".brandBtn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      syncBrandSwitcher_();
      // Re-render report live if currently viewing it
      if (document.querySelector("#page-report.active")) renderReport_();
    });
  });
...
if (_btnCloudSave){
  ...
  function sheet_(title){
    ...
    if (title){
      ...
      head.innerHTML = `<img class="lotusLogo" src="${logoSrc_()}" alt="${logoAlt_()}"/><img class="lotusLogo" src="${logoSrc_()}" alt="${logoAlt_()}"/>`;
  ...
  inner.insertAdjacentHTML("beforeend", `
    <div class="coverHeader">
      <img class="lotusLogo" src="${logoSrc_()}" alt="${logoAlt_()}" />
